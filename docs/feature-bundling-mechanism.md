...
KMP physical bundling controls what shared feature code goes into `SharedLogic.framework`. Native Swift feature UI is split separately through Swift Package targets.

The package defines one core target and one target per native feature UI:
...
Product package libraries compose only the Swift feature targets they need:
...
Each iOS app target has its own registry:
...
Add it only to the product package libraries that should include it:

```swift
.library(
    name: "AppOneIOSFeatures",
    targets: ["SharedIOSCore", "FeatureOrders", "FeatureCatalog", "FeatureProductDetails", "FeatureDelivery", "FeaturePayments"]
)
```

Update each product registry that includes the feature:
...
### 4. Remove Native iOS Swift Target Usage

Remove the feature target from the product package library:
...
Remove the feature module from the product registry:
...
The feature target can remain in the package if other products still use it.
...
- Swift package product targets match the native UI features included in that product.
- Product Swift registries include only available native feature modules.
...
| Swift UI falls back to generic feature UI | KMP exposes the feature, but the native iOS registry does not include the matching Swift feature module. | Add the native feature target and registry module for that product. |
