...
The same config maps feature ids to native Swift package targets and feature module enums:
...
That config is used for build metadata, display, generated feature bundle source, selected platform dependencies, Swift package product membership, and generated iOS feature registries.
...
`Package.swift` reads `config/product-feature-bundles.json` and derives package targets for each native feature UI module:

```swift
private let packageTargets: [Target] = [
    .target(name: "SharedIOSCore", dependencies: [...], path: "SharedIOS/Core"),
] +
...
It also derives product-level library products:
...
Each product owns a generated registry file:
...
4. Add `iosFeatureProductName` and `iosFeatureRegistryFile` to the product metadata.
5. Update Xcode synchronized folder exceptions so unused `SharedIOS/Features/*` folders are not target members.
6. Run `./gradlew generateIOSProductFeatureBundles` to regenerate product bundle constants and the native Swift registry.
